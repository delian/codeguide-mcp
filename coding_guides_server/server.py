...
def get_guide_from_cache(guide_name: str) -> Optional[str]:
    ...
    if cache_path.exists():
        try:
            ...
            logger.warning(f"Error reading cached guide {guide_name}: {e}")
    return None
...
def get_guides_list() -> str:
    ...
    if not guides and GUIDES_DIR.exists():
    ...
    if not guides:
        return "No guides found. Check network connection or local cache."
...
async def get_guide(guide_name: str) -> str:
    ...
    # Fall back to local cache
    logger.info(f"Using local cache or local directory for {guide_name}")
    cache_path = CACHE_DIR / guide_name
    if cache_path.exists():
        try:
