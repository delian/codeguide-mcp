    ...
    async with stdio_client(server_params) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            if not instructions or not (instructions or "").strip():
                errors.append("Server did not provide instructions (InitializeResult.instructions empty)")
                print("❌ FAIL: No instructions in InitializeResult")
            else:
                print(f"✅ PASS: Instructions provided ({len(instructions)} chars)")

            server_name = getattr(init_result.serverInfo, "name", None) if init_result.serverInfo else None
            if server_name:
            ...
            templates = list_tpl_result.resourceTemplates or []
            guide_template = None
            for t in templates:
            ...
            if not guide_template:
                template_uris = [getattr(t, "uriTemplate", None) or getattr(t, "uri_template", "") for t in templates]
                errors.append(f"Template guides://{{guide_name}} not exposed. Got: {template_uris}")
            ...
            try:
                ...
                if not contents:
                    ...
                    text = getattr(contents[0], "text", None) or (contents[0] if isinstance(contents[0], str) else None)
                    if not text:
            ...
            try:
                ...
                if not contents:
                    errors.append(f"read_resource(guides://{sample_guide}) returned no content")
                    print("❌ FAIL: No content")
                else:
                    c0 = contents[0]
                    text = getattr(c0, "text", None)
                    if text is None and hasattr(c0, "blob") and c0.blob is not None:
                        text = c0.blob.decode("utf-8") if isinstance(c0.blob, bytes) else str(c0.blob)
                    if not text:
                        text = str(c0)
                    if not text.strip().startswith("#"):
                        warnings.append(f"guides://{sample_guide} content unexpected")
                        print(f"⚠️  WARN: Unexpected content (starts with: {repr(text[:80])})")
                    else:
                        print(f"✅ PASS: guides://{sample_guide} usable (content length {len(text)})")
            except Exception as e:
            ...
            print("-" * 60)
            try:
                call_result = await session.call_tool("get_guide", {"guide_name": sample_guide})
                ...
                if not content_blocks:
                    ...
                    if "ERROR: Guide" in text:
                        ...
                        print(f"⚠️  WARN: get_guide returned error content")
                    elif not text.strip():
                        warnings.append("get_guide returned empty content")
                        print("⚠️  WARN: Empty content")
                    else:
                        print(f"✅ PASS: get_guide({sample_guide}) usable (content length {len(text)})")
            except Exception as e:
            ...
            print("\n" + "-" * 60)
            print(f"8. Non-existent guide via resource guides://{nonexistent} (expect error)...")
            print("-" * 60)
            try:
                await session.read_resource(f"guides://{nonexistent}")
                errors.append(f"read_resource(guides://{nonexistent}) should have failed but returned success")
                ...
                if "not found" in err_msg or "error" in err_msg or "resource" in err_msg:
                    ...
                    errors.append(f"read_resource(guides://{nonexistent}): unexpected error: {e}")
                    print(f"❌ FAIL: Unexpected error: {e}")

            # --- 9. Non-existent guide via get_guide tool must return error ---
            print("\n" + "-" * 60)
            print(f"9. Non-existent guide via tool get_guide({nonexistent}) (expect error)...")
            print("-" * 60)
            try:
                call_result = await session.call_tool("get_guide", {"guide_name": nonexistent})
                content_blocks = getattr(call_result, "content", None) or []
                parts = []
                for block in content_blocks:
                    if hasattr(block, "text"):
                ...
                is_error = "not found" in text.lower() and ("error" in text.lower() or "ERROR" in text)
                if is_error:
                    print(f"✅ PASS: get_guide returned error as expected")
                else:
                    errors.append(f"get_guide({nonexistent}) should return error message, got: {text[:200]!r}")
                    print(f"❌ FAIL: Expected error message in content, got: {text[:150]!r}")
            except Exception as e:
